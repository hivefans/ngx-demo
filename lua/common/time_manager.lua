...
--[[
    根据当前时间 + 随机数生成 唯一id
    格式: 01 20190501103055 254763 123456
    1~2: worker id
    3~16: 当前时间，年月日时分秒
    17~22: 当前时间的微妙
    23~28: 6位随机码 
...
function _M.uuid()
    local prefix = string.format('%02d', ngx.worker.id())
    local datetime = os.date("%Y%m%d%H%M%S",os.time())
    local microsecond = _M.microsecond()
    local rad_num = math.random(1,999999)
    return string.format("%s%s%06d%06d", prefix, datetime, microsecond, rad_num)
end
