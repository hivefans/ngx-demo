...
local sign_compare = function ( v1,v2 )
	...
	local iLen_1 = string.len(v1_str)
	local iLen_2 = string.len(v2_str)
...
local make_sign_str_sort
make_sign_str_sort = function( _params )
	-- body
	local t_src = {}
	-- 首先生成数组
	...
	for i=1,#t_src  do
		if  type(t_src[i][2]) == "table" then
			res = res..t_src[i][1].."={"..make_sign_str_sort(t_src[i][2]).."}"
		else
...
_M.make_sign_str_sort = make_sign_str_sort
...
-- 			local json_data = '{'
-- 			local t_sub_src = {}
-- 			for k,v in pairs(t_src[i][2]) do
-- 				table.insert(t_sub_src,{k,v})
-- 			end
-- 			table.sort(t_sub_src,sign_compare)
-- 			for i=1,#t_sub_src  do
-- 				json_data = json_data..t_sub_src[i][1]..'='..t_sub_src[i][2]
-- 				if i ~= #t_sub_src then
-- 					json_data = json_data.."&"
-- 				end
-- 			end
-- 			json_data = json_data..'}'
-- 			res = res..t_src[i][1].."="..json_data--cjson.encode(t_src[i][2])
-- 		else
-- 			res = res..t_src[i][1].."="..t_src[i][2]
-- 		end
-- 		if i ~= #t_src then
...
--[[
make_url_str 创建最后的字符串,字符串未做编码处理
]]
_M.make_url_str = function( _params )
	-- body
	local res = ""
	for k,v in pairs(_params) do
		if  type(v) == "table" then
			res = res..k.."=".. cjson.encode(v)
		else
			res = res..k.."="..v
		end
		res = res.."&"
