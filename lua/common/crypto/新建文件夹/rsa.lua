...
function _M:new_rsa_private(private_key, algorithm, pwd)
    local opts = {
        private_key = private_key or RSA_PRIV_KEY,
...
function _M:sign(priv_key, algorithm, pwd, buf) 
    local opts = {
    ...
    local sign, err = rsa_pub:sign(buf)
    if not sign then
...
function _M:verify(pub_key, algorithm, buf, sign)
    ...
    local verify, err = self.m_rsa:verify(buf, sign)
    if not verify then
