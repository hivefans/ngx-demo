...
local sys_conf = sys.sys_conf
local gapi_manager = require("gapi_platform.model.gapi_manager")
local api_def = require("gapi_platform.three_game.lc.api_def")
local time_help = require "resty.utils.time.time_help"
local sign_help = require "resty.utils.crypto.sign_help"
local aes = require "resty.utils.crypto.aes"
...
function _M.call(api_url, agent_id, des_key, md5_key, param)
    -- 获得当前时间毫秒数
    local timestamp = time_help.current_time_millis()

    -- aes 参数加密
    local str = sign_help.make_urlencode_str(param)
    local aes_param = aes.aes_ecb_encryp(des_key, str)
    -- md5 加密
    local key = hash_help.md5(string.format("%s%s%s", agent_id, timestamp, md5_key))
    ...
    local url = string.format("%s?%s", api_url, sign_help.make_url_str(data))
...
function _M.register(args, agent)
    return sys.response(sys.ERR_CODE.SUCCESS, "注册成功.")
end
...
function _M.login(args, agent)
    ...
    if not res then
        ...
        return sys.response(sys.ERR_CODE.SYS_ERR, '登入游戏失败.', { error = err})
    end

    -- 返回失败, 通知前端 服务器业务块
    if res.status ~= 200 then
        ngx.log(ngx.ERR, "登入游戏失败： user_id=", user_id, ", err=", res.body, ' status: ', res.status)
        return sys.response(sys.ERR_CODE.THIRD_PARTY_ERR, '登入游戏失败.', { error = res.body })
    end
    ...
    if code == api_def.ERR_CODE.SUCCESS and url then
        return sys.response(sys.ERR_CODE.SUCCESS, '登入游戏成功.', { url = url })
    else
        ngx.log(ngx.ERR, "登入游戏失败： user_id=", user_id, ", res=", res.body)
        return sys.response(sys.ERR_CODE.THIRD_PARTY_ERR, "登入游戏失败.", { error = "游戏平台返回错误", data = res_data})
    end
end
...
function _M.transfer_credit_in(args, agent)
    ...
    local user_id = args.user_id
    if not user_id then
        return sys.response(sys.ERR_CODE.PARAM_ERR, "参数错误", "[user_id]错误.")
    end
    ...
    if not res then
        ...
        if 'timeout' == err then
            return sys.response(sys.ERR_CODE.THIRD_PARTY_TIMEOUT, "请求第三方接口超时.")
        end
        return sys.response(sys.ERR_CODE.THIRD_PARTY_ERR, "请求第三方接口错误.")
    end
    ...
    if 200 ~= res.status then
        ...
        return sys.response(sys.ERR_CODE.THIRD_PARTY_ERR, "上分失败.")
    end
    ...
    if code == api_def.ERR_CODE.SUCCESS then
        return sys.response(sys.ERR_CODE.SUCCESS, "上分成功.")
    else
        ngx.log(ngx.ERR, 'resp: ', res.body)
        return sys.response(sys.ERR_CODE.THIRD_PARTY_ERR, "上分失败.", { error = "游戏平台返回错误", data = res_data})
    end
end
...
function _M.transfer_credit_out(args, agent)
    ...
    if not ok then
        ...
        return sys.response(sys.ERR_CODE.PARAM_ERR, "参数错误", {error = err})
    end
    ...
    if 200 ~= res.status then
        ...
        return sys.response(sys.ERR_CODE.THIRD_PARTY_ERR, "请求第三方接口失败.")
    end
    ...
    -- 0 表示成功 其余数值表示失败
    if code == api_def.ERR_CODE.SUCCESS then
        return sys.response(sys.ERR_CODE.SUCCESS, '下分成功.', { order_no = trade_no })
    else
        ngx.log(ngx.ERR, 'resp: ', res.body)
        return sys.response(sys.ERR_CODE.THIRD_PARTY_ERR, "下分失败.", { error = "游戏平台返回错误", data = res_data})
    end
end
...
function _M.get_blance(args, agent)
    ...
    -- 0 表示成功 其余数值表示失败
    if code == api_def.ERR_CODE.SUCCESS then
        return sys.response(sys.ERR_CODE.SUCCESS, "获取用户余额成功.", { balance = res_data.d.totalMoney })
    else
        ngx.log(ngx.ERR, string.format('游戏平台[%s]返回错误: %s', args.game_platform_code, res.body))
        return sys.response(sys.ERR_CODE.THIRD_PARTY_ERR, "获取用户余额失败.", {error = "游戏平台返回错误", data=res_data})
    end
end
...
function _M.get_transaction(args, agent)
    ...
    -- 0 表示成功 其余数值表示失败
    if code == 0 then
        return sys.response(sys.ERR_CODE.SUCCESS, "查询信息成功.", { state = state, msg = msg })
    else
        ngx.log(ngx.ERR, string.format('游戏平台[%s]返回错误: %s', args.game_platform_code, res.body))
        return sys.response(sys.ERR_CODE.THIRD_PARTY_ERR, "查询信息失败.", {error = "游戏平台返回错误", data=res_data})
    end
end
...
function _M.get_game_record(args, agent)
    ...
    -- 0 表示成功 其余数值表示失败
    if code == api_def.ERR_CODE.SUCCESS then
        --return sys.response(sys.ERR_CODE.SUCCESS, '获取游戏记录成功.')
    ...
    elseif code == api_def.ERR_CODE.DATA_NO_EXIST then
        ...
        --return sys.response(sys.ERR_CODE.THIRD_PARTY_ERR, "获取游戏记录失败.")
    end
end
...
function _M.kick_user(args, agent)
    ...
    -- 0 表示成功 其余数值表示失败
    if code == api_def.ERR_CODE.SUCCESS then
        return sys.response(sys.ERR_CODE.SUCCESS, '注销成功.')
    else
        ngx.log(ngx.ERR, 'resp: ', res.body)
        return sys.response(sys.ERR_CODE.THIRD_PARTY_ERR, "注销失败.")
    end
end
