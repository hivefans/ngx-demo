...
function _M:create(name, channel, handler)
    ...
    if handler and handler.on_event then
        handler.on_event("WS_EVENT_OPEN",o)
    end
    return o
end
...
function _M.on_publish(self, red)
    ...
    while true do
        ...
        if res then
            local item = res[3]
            local bytes, err = self.wb:send_text(item)
            if not bytes then
...
function _M:run(t_timeout)
    ...
    while true do
        ...
        -- 获取数据
        local data, type, err = self.wb:recv_frame()

        -- 如果连接损坏 退出
        if self.wb.fatal then
            ngx.log(ngx.ERR, "===>> Failed to receive frame: " .. err)
            if self.handler.on_event then
                self.handler.on_event("WS_EVENT_ERROR", self, err)
            end
            break
        end
        ...
        elseif type == "close" then
            if self.handler.on_event then
                self.handler.on_event("WS_EVENT_CLOSE", self)
            end
            break
        ...
        elseif type == "text" then
            ...
            --     self.wb.send_close(self.exit_code, self.exit_reason)
            --     break
            -- end
