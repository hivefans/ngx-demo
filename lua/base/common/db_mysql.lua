--[[ 
    @author: Created by Lixy at 2018-12-29 10:12
    @brief: MYSQL 数据库操作管理
--]]
...
local  MYSQL_CONNECT_TIME_OUT = 5000    
local  MYSQL_CONNECT_POOL_SIZE = 5000   -- 缓冲池大小


function _M:init(host, port, db_name, user, passwd, max_packet_size, timeout)
    self.conf = {
        host = host,
        port = port,
        database = db_name,
...
function _M:new(conf) 
    local db_conf = conf or self.conf
    if not db_conf then
        return nil, "The database configuration is not set."
    end

    local db = resty_mysql:new()
    if not db then
        ngx.log(ngx.ERR, "Create database object failed: resty mysql new() failed.")
        return nil, "Create database object failed."
    end

    db:set_timeout(self.t_timeout)
    local res, err = db:connect(db_conf)
    if not res then
        ngx.log(ngx.ERR, "Database Connect failed: ", err, "\n conf:", db_conf.host)
        return nil, "Database connect failed."
    end

    local res, err, errcode, sqlstate = db:query("SET NAMES UTF8;")
    if not res then
        ngx.log(ngx.ERR, "SET NAMES UTF8 failed: ", err)
    end
...
function _M:exec_query(sql)
    if not self.m_db then
        return nil, "The database is not connected."
    end

    local res, errmsg, errcode, sqlstate = self.m_db:query(sql)
    if not res then
        ngx.log(ngx.ERR, string.format(
            "Execute SQL failed: \n\t>>> msg= %s\n\t>>> code= %s\n\t>>> sqlstate= %s\n\t>>> sql= %s",
            errmsg, errcode, sqlstate, sql))
        
        if self.m_transaction > 0 then
            self.rollback()
        end
        return nil, "Execute SQL failed.", errcode, sqlstate
    end
    return res, nil, errcode, sqlstate
end


function _M:exec_once(sql, conf)
    local mysql, err =  self:new(conf)
    if not mysql then
        return nil, err
    end
    local res, errmsg, errcode, sqlstate = mysql:exec_query(sql)
    mysql:close()
    return res, errmsg, errcode, sqlstate
end

function _M:start_transaction()
	if not self.m_db then 
		return nil, "The datatase is not connected."
	end
    local res, err = self:exec_query("START TRANSACTION;")
    if res then
        self.m_transaction = self.m_transaction + 1
    end
    return res, err
end
...
function _M:rollback()
    ...
    local res, err = self:exec_query("ROLLBACK;")
    if res then
...
function _M:commit()
    ...
    local res, err = self:exec_query("COMMIT;")
    if res then
        self.m_transaction = 0
    end
    return res, err
...
function _M:close(keepalive)
    ...
    if self.m_transaction > 0 then
		local res, err = self:commit()
		if not res then
			return nil, err
		end
    end
    if keepalive then
        self.m_db:set_keepalive(self.t_timeout, self.pool_size)
    else
        self.m_db:close()
    end
    return true
...
function _M.fmt_insert()
end

function _M.fmt_update()
end

function _M.fmt_delete()
end


--[[
	@author: Created by Lixy at 2018-12-18 10:48
	@brief:	 拼接sql语句
	@param:
...
--[[
	@author: Created by Lixy at 2018-12-25 10:38
	@brief: 分页查询, 并将数据结果分页格式处理	
	@param:
        tb_name: 数据表名
        conditions: SQL语句条件, 例如: WHERE id='100'
		page: 当前页码
		page_size: 单页数据量
	@return:
]]
function _M.paging(tb_name, conditions, page, page_size, db)
    ...
    local sql = string.format("SELECT count(1) count FROM %s %s;", tb_name, conditions or '') 
	local res, err = db:exec_query(sql)
	if not res then
		ngx.log(ngx.ERR, "数据库错误: \n  >>> ERR= ", err, "\n  >>> SQL= ", sql)
		return nil, "数据库错误: err=" .. (err or 'nil')
	end
...
--[[
	@author: Created by Lixy at 2018-12-27 17:27
	@brief:	检查数据库表数据
	@param:
	@return:
]]
function _M.check_modify(tb_name, id, values, db)
	local buf1 = ""
	for k0, v0 in pairs(id) do
		if buf1 ~= "" then buf1 = buf1 .. " AND " end
		buf1 = buf1 .. buf1.format("%s='%s'", k0, v0)
	end
	local buf2 = ""
	for k1, v1 in pairs(values) do
		if buf2 ~= "" then buf2 = buf2 .. " or " end
		buf2 = buf2 .. string.format("%s='%s'", k1, v1)
	end
	local sql = "SELECT * FROM " .. tb_name .. " WHERE (" .. buf1 .. ")"
	if buf2 ~= "" then
		sql = sql .. " or (" .. buf2 .. ")" 
	end

	local db, err = db or _M:new()
	if not db then
		return nil, err
	end
	local res, err = _M.exec_query(sql, db)
	if not res then
		return nil, err
	end
