...
function _M:new(conf, t_timeout)
    ...
    if not res then
        ngx.log(ngx.ERR, "MySql connect error:",err,", errcode:",errcode)
        return nil, "Connect DB error"
    end

    local res, err, errcode, sqlstate = db:query("SET NAMES UTF8;")
    if not res then
        ngx.log(ngx.ERR, "MySql query error: " .. (err or "nil") .. ", errcode:" .. (errcode or "nil") .. ", sql_state:" .. (sqlstate or "nil"))
...
function _M:query(sql)
	...
	if not res then
		ngx.log(ngx.ERR, string.format("MySql error: %s, errcode=%s, sqlstate=%s", err, errcode, sqlstate))
	end
    return res, err, errcode, sqlstate
end
