...
function _M.connect_mod( self, redis )
    ...
    local ok, err = redis:connect(self.redis_cf.host, self.redis_cf.port)

    if self.auth_pwd then
        local res, errauth = redis:auth(self.auth_pwd)
        if not res then
            return nil ,errauth
        end 
    end
...
local default_redis_cfg = require "server.conf.redis_conf".redis_master_local

function _M.new(self, opts)
    opts = opts or {}
    local timeout = (opts.timeout and opts.timeout * 1000) or 1000
    local db_index= opts.db_index or 0
    local redis_cf = opts.redis_cf and opts.redis_cf or default_redis_cfg;

    return setmetatable({
            timeout = timeout,
            redis_cf = redis_cf,
            db_index = db_index,
            auth_pwd = redis_cf.password,
            _reqs = nil }, mt)


end

function _M:execute(red, cmd, ...)
    local red = red or _M:new()
    if not red then
        return nil, "Create redis object failed"
    end
    local redis_cmd = red[cmd]
    if not redis_cmd then
        ngx.log(ngx.ERR, "[REDIS ERROR] " .. "redis command [" .. (cmd or "nil") .. "] is not exist.")
        return nil, "redis command [" .. cmd .. "] is not exist."
    end
    local res, err = redis_cmd(red, ...)
    if not res then
        ngx.log(ngx.ERR, "[REDIS] cmd=" .. cmd .. ", err=" .. (err or "no data"))
    end
    return res, err
end

function _M:exec_hgetall(key)
    local redis_cli = _M:new();
    if not redis_cli then
        ngx.log(ngx.ERR, "=====>>> REDIS服务器异常")
        return false, 'REDIS服务器异常'
    end
    local res = redis_cli:hgetall(key);
    if not res then
        ngx.log(ngx.ERR, "读取REDIS：key:["..key.."]不存在")
        return {}, "无记录"
    end
    return res
end
...
function _M:exec_hset(key, field, value)
    local redis_cli = _M:new();
    if not redis_cli then
        ngx.log(ngx.ERR, "=====>>> REDIS服务器异常")
        return false, 'REDIS服务器异常'
    end
    local res = redis_cli:hset(key, field, value);
    if not res then
        ngx.log(ngx.ERR, "写入REDIS失败：key=["..key.."]不存在")
        return false, "写入REDIS失败：key=["..key.."],field=["..field.."],value=["..value.."]"
    end
    return res, "写入REDIS成功：key=["..key.."],field=["..field.."],value=["..value.."]"
end


function _M:exec_hget(key, field)
    local redis_cli = _M:new();
    if not redis_cli then
        ngx.log(ngx.ERR, "=====>>> REDIS服务器异常")
        return false, 'REDIS服务器异常'
    end
    local res = redis_cli:hget(key, field);
    if not res then
        ngx.log(ngx.ERR, "HGET REDIS失败：key=["..key.."], field=["..field.."]")
        return false, "HGET REDIS失败：key=["..key.."],field=["..field.."]"
    end
    ngx.log(ngx.ERR, "HGET REDIS成功：key=["..key.."], field=["..field.."]")
    return res
end

function _M:exec_hdel(key, field)
    local redis_cli = _M:new();
    if not redis_cli then
        ngx.log(ngx.ERR, "=====>>> REDIS服务器异常")
        return false, 'REDIS服务器异常'
    end
    local res = redis_cli:hdel(key, field);
    if not res then
        ngx.log(ngx.ERR, "HDEL REDIS失败：key=["..key.."], field=["..field.."]")
        return false, "HDEL REDIS失败：key=["..key.."],field=["..field.."]"
    end
    ngx.log(ngx.ERR, "HDEL REDIS成功：key=["..key.."], field=["..field.."]")
    return res
end
