...
function table.arrayMerge( tDest, tSrc )
    ...
    if not tDest then return tSrc end
...
-- @param tSrc 来源表格(t表示是table)
--]]
function table.arrayRemove( tSet, tSubSet, opt )
    if not tSet or not tSubSet then
        return nil
    end
    local dLen = table.getn(tSet);
    local sLen = table.getn(tSubSet);
    local tDest = {};
    local tIndex = 1;
    for i = 1, dLen,1 do
        local fTemp = tSet[i];
        for j = 1, sLen, 1 do
            if opt then
                fTemp = opt(fTemp,tSubSet[j]);
            else
                fTemp =  (fTemp == tSubSet[j] and nil or fTemp);
            end
            if not fTemp then
                break;
            end
        end
        -- 如果ftemp 存在 则ftemp 设置为 nil ;不为nil的说明不存在 ,添加进入不存在的目录
        if fTemp then
            tDest[tIndex] = fTemp;
            tIndex = tIndex+1;
        end 
...
require "resty.lua_ex.lua_str_ex"
