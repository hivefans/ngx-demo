...
function  _M:sendMsg( _msg, _msgType )
	-- body
	if not _msg then
		local msgData = api_response.response_err(401,"数据格式出错")
	...
	while self.closeFlag == false do
	    ...
	    elseif typ == "ping" then
	        ...
	        if not bytes then
	            ...
	            -- 通知业务引擎进行状态提醒和修改
				msgFCB(WS_EVENT.PING_ERR_EVENT)
	             return ngx.exit(444)
	        end
	    ...
	    elseif typ == "text" then
			...
			local res, taskSign = msgFCB(WS_EVENT.TEXT_EVENT,data)
			if res then 
		        ...
		        if not bytes then
	        		...
	        		-- 通知业务引擎进行状态提醒和修改 
		           	local res = msgFCB(WS_EVENT.SEND_ERR_EVENT,taskSign)
